...
LOG_FILE_PATH = str((Path.home() / ".forgepilot" / "logs" / "forgepilot.log").resolve())
...
{"type":"plan","goal":"...","steps":[{"id":"1","description":"..."},{"id":"2","description":"..."}],"notes":"..."}
...
def create_session(phase: str = "plan") -> AgentSession:
    ...
    session = _build_session(session_id, phase, datetime.utcnow(), aborted=False)
    _local_sessions[session_id] = session
    loop.create_task(create_runtime_session(session_id, phase))
    return session
...
def delete_session(session_id: str) -> bool:
    ...
    if local:
        local.abort_event.set()
    loop.create_task(delete_session_async(session_id))
...
async def _wait_for_permission_decision(session_id: str, permission_id: str) -> bool:
    ...
    while True:
        ...
        await asyncio.sleep(PERMISSION_POLL_INTERVAL_SECONDS)
...
async def respond_to_permission_async(session_id: str, permission_id: str, approved: bool) -> bool:
    ...
    return await set_runtime_permission_status(session_id, permission_id, next_status)
...
def save_plan(plan: dict[str, Any]) -> None:
    ...
    if plan_id:
        _local_plans[plan_id] = plan
    loop.create_task(save_plan_async(plan))
...
3. Use Read before Write even for new files
"""
    if sandbox_enabled:
        base += """
## Sandbox Mode
