...
def _find_srt_path() -> str | None:
    env_path = os.getenv("SRT_PATH")
    if env_path and Path(env_path).exists():
        return env_path
    return shutil.which("srt")


class ClaudeProvider(ISandboxProvider):
    type = "claude"
    name = "Claude Sandbox"
    ...
    async def is_available(self) -> bool:
        self._srt_path = _find_srt_path()
        return self._srt_path is not None
    ...
    async def exec(self, options: SandboxExecOptions) -> SandboxExecResult:
        ...
        if not self._srt_path:
            return SandboxExecResult(stdout="", stderr="Sandbox Runtime is not installed", exit_code=1, duration=0)
        ...
        cmd = [self._srt_path, "run", "--", options.command, *args]
