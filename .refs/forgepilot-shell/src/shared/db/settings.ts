...
export interface Settings {
  ...
  mcpAppDirEnabled: boolean; // Enable loading MCP servers from app directory (forgepilot config)
...
export const defaultProviders: AIProvider[] = [
  ...
  },
  {
    ...
    apiType: 'anthropic-messages',
    icon: '3',
    apiKeyUrl: 'https://302.ai/?utm_source=forgepilot_desktop',
    canDelete: true,
  },
  {
    id: 'ollama',
...
// to platform-specific paths (e.g., ~/Library/Application Support/forgepilot on macOS)
...
const DB_NAME = 'sqlite:forgepilot.db';
...
export async function getSettingsAsync(): Promise<Settings> {
  ...
  try {
    const stored = localStorage.getItem('forgepilot_settings');
    if (stored) {
      ...
      console.log('[Settings] localStorage has no forgepilot_settings');
    }
  } catch (error) {
...
export async function saveSettingsAsync(settings: Settings): Promise<void> {
  ...
  try {
    localStorage.setItem('forgepilot_settings', JSON.stringify(settings));
  } catch (error) {
...
function isModelConfiguredFromSettings(settings: Settings): boolean {
...
type BackendProviderConfigResponse = {
  defaultProvider?: string;
  defaultModel?: string;
  agent?: {
    config?: {
...
async function hydrateModelConfigFromBackend(
  settings: Settings
): Promise<boolean> {
  if (isModelConfiguredFromSettings(settings)) {
    return false;
  }
  ...
  try {
    ...
    const payload = (await response.json()) as BackendProviderConfigResponse;
    const agentConfig = payload.agent?.config;
    const apiKey = (agentConfig?.apiKey || '').trim();
    const baseUrl = (agentConfig?.baseUrl || '').trim();
    const model = (agentConfig?.model || payload.defaultModel || '').trim();
    const apiType = (agentConfig?.apiType || 'openai-completions') as ApiType;

    // If backend doesn't have usable credentials, don't mutate local settings.
    if (!apiKey || !model) {
      return false;
    }

    const providerId = (payload.defaultProvider || 'codex_local').trim();
    const existingIndex = settings.providers.findIndex((p) => p.id === providerId);
    const existingProvider =
      existingIndex >= 0 ? settings.providers[existingIndex] : null;

    const mergedModels = Array.from(
      new Set([...(existingProvider?.models || []), model])
    );

    const hydratedProvider: AIProvider = {
      id: providerId,
      name: existingProvider?.name || 'Codex Local',
      apiKey,
      baseUrl: baseUrl || existingProvider?.baseUrl || '',
      enabled: true,
      models: mergedModels,
      defaultModel: model,
      apiType: apiType || existingProvider?.apiType || 'openai-completions',
      icon: existingProvider?.icon || 'C',
      apiKeyUrl: existingProvider?.apiKeyUrl,
      canDelete: existingProvider?.canDelete ?? true,
    };

    if (existingIndex >= 0) {
      settings.providers[existingIndex] = hydratedProvider;
    } else {
      settings.providers.push(hydratedProvider);
    }
    ...
    if (runtimeIndex >= 0) {
      settings.agentRuntimes[runtimeIndex] = {
        ...
        enabled: true,
        config: {
          ...settings.agentRuntimes[runtimeIndex].config,
          apiKey,
          baseUrl: hydratedProvider.baseUrl,
          model,
          apiType,
        },
      };
    ...
    console.warn('[Settings] Failed to hydrate model config from backend:', error);
    return false;
  }
}
...
export async function initializeSettings(): Promise<Settings> {
  ...
  if (await hydrateModelConfigFromBackend(settings)) {
    changed = true;
  }

  settingsCache = settings;

  // Persist any startup migrations / hydration to both SQLite and localStorage.
  if (changed) {
...
export function isModelConfigured(): boolean {
  const settings = getSettings();
  return isModelConfiguredFromSettings(settings);
}
...
export async function saveSettingItem(
  ...
  try {
    localStorage.setItem(`forgepilot_${key}`, value);
  } catch (error) {
...
export async function getSettingItem(key: string): Promise<string | null> {
  ...
  try {
    return localStorage.getItem(`forgepilot_${key}`);
  } catch {
    return null;
  }
}
