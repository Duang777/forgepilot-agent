/**
 * Path utilities for ForgePilot
 *
 * Uses ~/.forgepilot/ as the standard data directory across all platforms.
...
/**
 * Get the ForgePilot MCP config path (platform-aware)
 */
export async function getForgepilotMcpPath(): Promise<string> {
 ...
 * Returns ~/.forgepilot on all platforms (using correct path separator)
 */
export async function getAppDataDir(): Promise<string> {
  ...
  if (isTauri()) {
    try {
      ...
      const homeClean =
        home.endsWith('/') || home.endsWith('\\') ? home.slice(0, -1) : home;
      cachedAppDataDir = `${homeClean}${separator}.forgepilot`;
      return cachedAppDataDir;
    } catch (error) {
...
export async function getLogPath(): Promise<string> {
  const appDir = await getAppDataDir();
  const sep = await getPathSeparator();
  return `${appDir}${sep}logs${sep}forgepilot.log`;
}
