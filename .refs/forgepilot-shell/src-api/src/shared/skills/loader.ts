...
import { getClaudeSkillsDir, getForgepilotSkillsDir } from '@/config/constants';
...
/**
 * Install built-in skills from project resources to ~/.forgepilot/skills/
...
export async function installBuiltinSkills(): Promise<void> {
  ...
  const targetDir = getForgepilotSkillsDir();
...
/**
 * Load skills from all directories (both ~/.claude/skills/ and ~/.forgepilot/skills/)
 */
export async function loadAllSkills(
  ...
  const skills: LoadedSkill[] = [];
  const dirs = [getClaudeSkillsDir(), getForgepilotSkillsDir()];
