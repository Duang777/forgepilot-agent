 ...
 * MCP Config Loader
 *
 * Loads MCP server configuration from ~/.forgepilot/mcp.json
 */

import fs from 'fs/promises';

import { getForgepilotMcpConfigPath } from '@/config/constants';
...
// ForgePilot MCP Config file format
interface _ForgePilotMcpConfig {
  mcpServers: Record<
    string,
    {
...
export function getMcpConfigPath(): string {
  return getForgepilotMcpConfigPath();
}
...
/**
 * Load MCP servers configuration from ~/.forgepilot/mcp.json
 *
...
export async function loadMcpServers(
  ...
  const servers = await loadMcpServersFromFile(configPath, 'forgepilot');
